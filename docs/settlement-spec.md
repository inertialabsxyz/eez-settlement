...
4. **Atomicity under stress.** *Closed.* `script/e2e.sh unwind` dispatches a settlement L1 must
   reject and asserts that nothing moved on either chain. Observed on a live devnet:

   Alice submits a genuine intent for 1,000 USDC and `Book` accepts it. The solver then reveals a
   batch carrying a signature she made over **999 USDC**. L2 cannot detect the substitution —
   `submitIntent` verified a signature and discarded it (§5.1), and `_validateAndScore` never reads
   one, checking only the trade against stored intent state, which matches exactly. So L2 accepted
   the payload, scored it at 160.0, and dispatched. `Executor._verifyAndPull` re-derived the digest
   on L1, recovered a different address, and reverted (I9).

   Every write unwound with it: the auction was still unsettled, intent 3 still `Live` rather than
   `Filled`, alice's nonce 2 unspent on L1, and her balance, `Executor`'s and `windfallRecipient`'s
   all exactly where they started. The intent remained available to any later solver, so the failed
   attempt cost the solver their gas and nobody else anything (§7.2).

   Two notes from the run. The nonce is written *before* the signature check in `_verifyAndPull`, so
   it is the revert rather than the ordering that leaves it unspent — worth knowing if that ordering
   is ever changed. And the cross-chain front's nonce did **not** advance within 150s on the failing
   path, while the assertions passed regardless: the front's nonce is a "no longer in flight" signal
   and reveals nothing about the outcome, which is why every assertion here reads a chain fact.
